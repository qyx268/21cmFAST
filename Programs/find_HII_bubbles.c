...
int main(int argc, char ** argv){
...
#ifdef MINI_HALO
  float F_STAR10m, F_ESC10m, Mlim_Fstarm; //New in v1.5
  float M_MINm, M_MINa; //New in v1.5
#endif
  double X_LUMINOSITY;
  ...
  *error_message = '\0';
  ...
  if (HALO_MASS_DEPENDENT_IONIZING_EFFICIENCY) {
    ...
    Mlim_Fstar  = Mass_limit_bisection(M_MINa/50., 1e16, ALPHA_STAR, F_STAR10);
    Mlim_Fesc   = Mass_limit_bisection(M_MINa/50., 1e16, ALPHA_ESC, F_ESC10);
    Mlim_Fstarm = Mass_limit_bisection(M_MINm/50., 1e16, ALPHA_STAR, F_STAR10m);
    ION_EFF_FACTOR      = N_GAMMA_UV      * F_STAR10  * F_ESC10;
    ION_EFF_FACTOR_MINI = N_GAMMA_UV_MINI * F_STAR10m * F_ESC10m;
#else
    M_MIN = M_TURN;
    Mlim_Fstar = Mass_limit_bisection(M_MIN/50., 1e16, ALPHA_STAR, F_STAR10);
    Mlim_Fesc  = Mass_limit_bisection(M_MIN/50., 1e16, ALPHA_ESC, F_ESC10);
    ION_EFF_FACTOR = N_GAMMA_UV * F_STAR10 * F_ESC10;
#endif
  }
  else{
...
#ifdef MINI_HALO
    // Nion_ST * ION_EFF_FACTOR + Nion_ST_MINI * ION_EFF_FACTOR_MINI = the mean number of IGM ionizing photon per baryon
    mean_f_coll_st  = Nion_ST(REDSHIFT, M_MINa,   ALPHA_STAR, ALPHA_ESC, F_STAR10, F_ESC10, Mlim_Fstar, Mlim_Fesc);
    mean_f_coll_stm = Nion_ST_MINI(REDSHIFT, M_MINm, Mcrit_atom, ALPHA_STAR, F_STAR10m, F_ESC10m, Mlim_Fstarm);
#else
...
#ifdef MINI_HALO
#else
      initialise_Nion_splinem(REDSHIFT, massofscaleR,M_MINa,ALPHA_STAR,ALPHA_ESC,F_STAR10,F_ESC10,Mlim_Fstar,Mlim_Fesc,M_MINm,Mcrit_atom,F_STAR10m,F_ESC10m,Mlim_Fstarm);
#else
